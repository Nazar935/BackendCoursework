...
class DB
{
    ...
    public function select(string $table, array $fields = ['*'], array $conditions = null,
                           bool $or = false, int $page_i = 0, int $page_offset = 1000000, string $order = '', $search = null): array
    {
        ...
        $res = $this->pdo->prepare("select $fieldsStr from $table $whereStr $orderStr limit $page_i, $page_offset");
    ...
    public function deleteWithConditions(string $table, array $conditions): bool
    {
        $whereStr = $this->double_implode($conditions, ' and ');
        $res = $this->pdo->prepare("DELETE FROM $table WHERE $whereStr");
        return $res->execute($conditions);
    }
